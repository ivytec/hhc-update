...
本软件可以替代罗斯蒙特475以及其他类似的HART手操器，并支持无线操作、版本自动更新、中/英文菜单、过程变量趋势图、仪表告警实时检测等其他手操器无法提供的功能，兼容国内外主流的 压力仪表、温度仪表、流量计、物位计等近200种HART协议智能仪表。

无线操作
:   通过Wi-Fi，操作人员可以远距离对仪表进行配置，更安全方便。

支持仪表全面
:   支持进口（如罗斯蒙特、E+H等）、国产（如肯特、北京华控、江苏华海、富沃得等）各种智能压力、温度、流量、物位变送器的组态、标定、诊断、维护、监视。常用设置：单位、量程、空满标、微调、信号发生器、小信号切除、传感器类型修改等。

:   全面支持国内各厂家的电磁、金转、V锥、靶式、涡街流量计。

支持进口仪表高级功能
:   目前已经支持40多种进口仪表高级调试(菜单同罗斯蒙特475)，未来会持续增加，只需更新软件即可使用；已支持的进口仪表系列或型号：
...
自动更新版本
:   升级不再依赖电脑和光盘，通过内置的一键升级功能，便可自动更新至最新的软件版本，简单快捷，不易出错。

中/英文菜单
:   支持中文和英文菜单，且每项菜单中均有对应的帮助说明，详细介绍该项功能的含义及使用方式；菜单结构基本与罗斯蒙特475一致，可根据使用习惯一键切换菜单语言。
...
运行指示灯  | 绿    | 慢闪+快闪  | AP模式，快闪表示数据收发
运行指示灯  |       | 常亮  | 设备异常，需要人工复位
运行指示灯  | 红    | 慢闪  | 站点模式，等待手操器连接
运行指示灯  | 红    | 快闪  | 站点模式，正在连接网络
运行指示灯  | 红    | 慢闪+快闪  | 站点模式，快闪表示数据收发
...
步骤1
:  S市工厂操作人员：将适配器连接上仪表，并让本软件连接到适配器的网络；

步骤2
:  S市工厂操作人员：确保手机的数据通道（2G、3G或4G皆可）可用，点击顶部工具栏折叠菜单（图标![overflow][overflow]）下的`请求远程协助`，在新界面下按照提示启动远程协助，启动成功后，会显示6位数字的配对码，请将该配对码告知W市的远程协助方；

步骤3
:  W市工程公司人员：确保手机能够连接互联网，点击顶部工具栏折叠菜单（图标![overflow][overflow]）下的`连接远程适配器`，输入配对码，连接成功后，便可对远程仪表进行操作。
> **提示：** 远程操作受限于网络速度，响应可能会比较慢，建议操作不要太频繁。
...
　　
## 中/英文菜单
点击顶部工具栏菜单（图标![overflow][overflow]）下的`中英文切换`，可以一键切换中/英文菜单。
中英文的描述与475手操器大同小异，操作人员可以根据自己的习惯选择语种；每项菜单中均有详细的帮助说明（英文没有帮助说明），描述该项的作用及基本用法，更方便操作人员对仪表进行配置。

## 软件升级
打开HART手操器软件的`帮助`（在图标![overflow][overflow]下）界面，点击`软件版本`即可自动检查新版本并下载；下载期间，请不要退出软件，下载完成后，请根据提示安装，安装完成后，第一次运行新版本软件可能需要初始化运行数据，请耐心等候。

## 授权更新
没有授权，您无法使用本软件对仪表进行配置维护。
打开HART手操器软件的`帮助`（在图标![overflow][overflow]下）界面，即可看到`授权状态`，如果不是`已授权`状态，您可以点击`申请授权`直接申请，或在软件提示您授权不支持时，通过在线购买来获得授权；在授权文件准备好后，我们会通过电话或E-Mail通知您，您可以点击`更新授权`更新授权文件。

## 操作日志
操作日志记录了用户在HART手操器软件中的每一步操作，方便用户分析与回溯。
在系统设置（在图标![overflow][overflow]下）界面下，点击`操作日志`，便可看到操作日志的入口；打开操作日志后，您还可以通过点击工具栏图标![send][send]，以蓝牙、E-Mail、或其他您的设备支持的方式将操作日志发送到外部。
...
-  检查手机或平板与适配器距离是否太远，建议适配器的Wi-Fi信号保持在两格以上；
-  检查适配器运行灯是否常亮，如果常亮则表示运行异常，可常识给适配器重新上电；
